...
AttnA = net.Attn()
AttnB = net.Attn()
AttnA.apply(utils.weights_init)
AttnB.apply(utils.weights_init)
...
# pre=torch.load('./data/vgg16-397923af.pth')
...
###################################
        # GAN loss
        #A2B
        attnMapA = AttnA(real_A)
        real_attn_A = attnMapA * real_A
        fake_B = A2BG(real_attn_A)
        pred_fake = BD(fake_B)
        ...
        #B2A
        attnMapB = AttnB(real_B)
        real_attn_B = attnMapB * real_B
        fake_A = B2AG(real_attn_B)
        pred_fake = AD(fake_A)
    ...
    torch.save(AttnA.state_dict(), os.path.join(args.name + '_results', 'AttnA_latest.pkl'))
    torch.save(AttnB.state_dict(), os.path.join(args.name + '_results', 'AttnB_latest.pkl'))
